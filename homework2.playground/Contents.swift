...
/*
 ...
 min t: значення мінімальної темпераутри у Цельсій / значення
 мінімальної темпераутри у Фаренгейт
 max t: значення максимальної темпераутри у Цельсій /
 значення максимальної темпераутри у Фаренгейт
 наприклад:
 Kyiv:
 t: 25.0 C
 t: 65.00 F
 ...
 - ЕКРАН 1 виводить інформацю про погоду тільки у значенні
 Кельвін,
 ...
 у форматі:
 Назва міста:
 t: значення темпераутри у Цельсій наприклад:
 
 Kyiv:
 t: 25.0 C
 Поточний результат:
 - ЕКРАН 2 виводить інформацю тільки для міста Львів
 - ЕКРАН 2 виводить дублювання інформації для міста Львів у кількості
 білше, ніж кількість заданих міст разом
 ...
 Поточний результат:
 - ЕКРАН 3 виводить
 темпераутри у Фаренгейт
 інформацю тільки для одного міста
 і це місто змінюється випадковим чином після кожного запуску
 програми
 - ЕКРАН 3 виводить дублювання інформації одного випадеового міста у
 кількості 4 рази
 - ЕКРАН 3 виводить інформацю
 а має виводити у Фаренгейт
 Корисна інформація:
 *
 формула конвертації з Кельвіна
 Тц = Тк - 273.15,
 де Тц - температура у Цельсій
 Тк - температура у Кельвін
 *
 про погоду тільки у значенні Кельвін,
 у Цельсій:
 формула конвертації з Кельвіна
 ...
 Тк - температура у Кельвін
 *
 arc4random() % n - генерація випадкового числа у діапазоні від нуля до
 n,
 ...
 *
 ...
 де n - кількість числел для виводу після крапки наприклад:
 let somValue: Double = 23.34563725647
 let stringToPrint = String(format: "%.3f", somValue) // 23.345 23.345 - буде виведено значення: 23 і 3 числа після крапки
 *
 '_' - назва пам'яті, яка не використовується далі у коді (можна
 назвати заглушка)
 наприклад:
 var _ = index
 або
 let _ = index - значення індексу записується у константу, яка не
 використвується,
 Warnings (попередження,
 далі в коді,
 */
//може застосовуватися для уникання підсвітки наприклад, оголосили змінну, але не використали Xcode пдсвітить цю строку жовтим кольором)
...
typealias Weather = (city: String, temp: Double, tempMin: Double, tempMax: Double)
...
// запускаємо цикл для заповнення пустого масиву weatherInCities // інформацією про погоду для кожної назви міста, вказаних у масиві cityNames

let cityNamesCount: Int = cityNames.count

for index in 0 ..< cityNamesCount {
    
    let city = cityNames[index] // записуємо назву міста
    
    let temp = Double(arc4random() % 30) + kelvinZero // генеруємо випадкове значення температури у Кельвінах
    
    let tempMin = Double(arc4random() % 30) + kelvinZero //випадкове значення температури у Кельвінах
    
    let tempMax =  Double(arc4random() % 30) + kelvinZero//випадкове значення температури у Кельвінах
    
    let weatherInfo = Weather(city, temp, tempMin, tempMax)
...
let wetherIntCitiesCount: Int = weatherInCities.count

for index in 0 ..< wetherIntCitiesCount {
    
    let weatherInfo = weatherInCities[index]
    
    let city = weatherInfo.city
    let kelvin = weatherInfo.temp
    let kelvinMin = weatherInfo.tempMin
    let kelvinMax = weatherInfo.tempMax
...
print("\n-----------------------------")
print("\n\n")
// виводимо інформацію про температуру тільки у Цельсій // для кожного міста, що є у масиві weatherInCities print("---------- ЕКРАН 2 ----------")
for index in 1 ..< 10 {
    let _ = index
...
print("\n-----------------------------")
print("\n\n")
// виводимо інформацію про температуру тільки у Фаренгейт // для кожного міста, що є у масиві weatherInCities print("---------- ЕКРАН 3 ----------")
